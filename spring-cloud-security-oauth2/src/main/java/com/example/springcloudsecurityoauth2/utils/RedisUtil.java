package com.example.springcloudsecurityoauth2.utils;
...
public class RedisUtil {
    ...
    public  List<AppUserinfoEntity> rangeAppUserinfoEntity(String key, long start, long end) {
        List<Object> list =  redisTemplate.opsForList().range(key, start, end);
        return list.stream().map(this::convertToAppUserinfoEntity).collect(Collectors.toList());
    }

    /**
     * 将 Object 转换为 AppUserinfoEntity 对象。
     * 这里需要根据实际情况实现转换逻辑，比如反序列化 JSON 字符串。
     * @param object 要转换的对象
     * @return 转换后的 AppUserinfoEntity 对象
     */
    private AppUserinfoEntity convertToAppUserinfoEntity(Object object) {
        // 这里是转换逻辑的实现，比如：
        ...
        if (object instanceof String) {
            ...
            try {
                return new ObjectMapper().readValue(json, AppUserinfoEntity.class);
            } catch (JsonProcessingException e) {
                e.printStackTrace();
            }
        }
        // 如果没有合适的转换，可以抛出异常或返回 null
        throw new IllegalArgumentException("Cannot convert object to AppUserinfoEntity: " + object);
    }
