...
@Component
@Transactional
@RocketMQMessageListener(consumerGroup = "MessageConsumerMergeRequestGroup3", topic = "RED_PACKAGE_RAIN_MERGERE_QUEST_TOPIC",consumeMode= ConsumeMode.CONCURRENTLY)
public class MessageConsumerMergeRequest3 implements RocketMQListener<String> {
    @Autowired
    ...
    public void onMessage(String message) {
        try {
            ...
            // 处理消息
            log.info(StringUtil.StringAppend("MessageConsumerMergeRequestGroup3,收到第",recievedMessageCount,"条消息！！！收到消息: ",message));
            ...
            if (!StringUtils.isEmpty(messageId) && !StringUtils.isEmpty(message) && RedisUtil.getLock(RedPackageRainConstant.RED_PACKAGE_MESSAGE_ID + messageId,message,3600)) {
                log.info("MessageConsumerMergeRequestGroup3处理消息：" + messageId);
