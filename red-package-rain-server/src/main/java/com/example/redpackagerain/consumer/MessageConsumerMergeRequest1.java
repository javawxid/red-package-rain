...
@Component
@Transactional
@RocketMQMessageListener(consumerGroup = "MessageConsumerMergeRequestGroup1", topic = "RED_PACKAGE_RAIN_MERGERE_QUEST_TOPIC",consumeMode= ConsumeMode.CONCURRENTLY)
public class MessageConsumerMergeRequest1 implements RocketMQListener<String> {
    @Autowired
    ...
    public void onMessage(String message) {
        try {
            ...
            // 处理消息
            log.info(StringUtil.StringAppend("MessageConsumerMergeRequestGroup1,收到第",recievedMessageCount,"条消息！！！收到消息: ",message));
            ...
            if (!StringUtils.isEmpty(messageId) && !StringUtils.isEmpty(message) && RedisUtil.getLock(RedPackageRainConstant.RED_PACKAGE_MESSAGE_ID + messageId,message,3600)) {
                log.info("MessageConsumerMergeRequestGroup1处理消息：" + messageId);
